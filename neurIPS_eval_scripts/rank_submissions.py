...
def load_run_results(run_result_dir:str, hidden_dataset = False):
 
    results = {}

    for filename in os.listdir(run_result_dir):
        if not hidden_dataset and "hidden" in filename:
            continue
        
        if hidden_dataset and not "hidden" in filename:
            continue
...
def transpose_results(results):
    data_sets = ['CNN/DailyMail', "sam_sum", "corr2cause", 'ethics', 'MATH', "MMLU", "TruthfulQA", "BIG-bench", "GSM8K", "BBQ"]
    transposed_results = {}
...
if __name__ == "__main__":
    ...
    try:
        ...
        METRICS = Open_dataset_centric_metrics
        
        name = 'open'
        if args.hidden:
            name = 'hidden'
            METRICS = Hidden_dataset_centric_eval_metrics
            submission_results =load_run_results(args.dir, hidden_dataset=True)
        else:
        ...
        with open (f"{name}_rank.json", 'w') as handle:
            json.dump( ranked_results, handle, indent=4)

        print(f"{name}_rank.json")
